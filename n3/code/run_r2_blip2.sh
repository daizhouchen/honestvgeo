...
set -uo pipefail
mkdir -p ${PROJECT_ROOT}/n3/runs/R2_blip2_rerank
exec > ${PROJECT_ROOT}/n3/runs/R2_blip2_rerank/r2_blip2_full.log 2>&1
...
MAN=n3/data/esci1500_thumbs/manifest.csv
IMG=n3/data/esci1500_thumbs/img
...
run_one () {
  ...
  python n3/code/eval_harness_blip2.py --manifest "$MAN" --img-dir "$IMG" \
    --adv-dir "$adv" --out-dir "n3/runs/R2_blip2_rerank/$m" --method-tag "$m" --topk 50 \
    && echo "=== METHOD $m DONE $(date -u +%FT%TZ) ===" \
    || echo "=== METHOD $m FAIL $(date -u +%FT%TZ) ==="
}

run_one cogeo    n3/runs/T3_scaleup/run4/cogeo/img
run_one pgd_bare n3/runs/T3_scaleup/run4/pgd_bare/img
run_one coattack n3/runs/B2_full4method/eps16_1430/coattack/img
run_one advclip  n3/runs/B2_full4method/eps16_1430/advclip/img
