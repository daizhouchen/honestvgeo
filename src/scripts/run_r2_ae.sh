...
OUT=runs/R2_blip_rerank_ae
mkdir -p "$OUT"
exec > "$OUT/r2_ae.log" 2>&1

IMG=data/esci500_thumbs/img
AE=runs/ae_v3_ens4/img
...
df = pd.read_csv("data/esci500_thumbs/manifest.csv")
ae = "runs/ae_v3_ens4/img"
m = df[df["product_id"].apply(lambda a: os.path.exists(os.path.join(ae, f"{a}.jpg")))].copy()
m.to_csv("runs/R2_blip_rerank_ae/manifest_ae.csv", index=False)
print("ae_rows", len(m), "of", len(df))
PY

echo "=== AE-CoGEO (transfer) on BLIP $(date -u +%FT%TZ) ==="
python "$SRC_ROOT/eval/eval_harness_blip.py" --manifest "$MM" --img-dir "$IMG" \
