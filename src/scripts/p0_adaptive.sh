...
python "$SRC_ROOT/attacks/n3_attack.py" --method cogeo --manifest "$MAN" --img-dir "$IMG" \
...
python "$SRC_ROOT/defense/purify_dir.py" --src-dir "$ADV" --out-dir "$OUT/adv_adapt_jq40" --mode jpeg --param 40
...
python "$SRC_ROOT/eval/eval_harness.py" --mode rank --manifest "$MAN" --img-dir "$CLEAN_JQ40" \
