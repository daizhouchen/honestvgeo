...
class EnvSim(nn.Module):
    ...
    def __init__(self, p: float = 0.5, jpeg_quality: int = 85, tim_k: int = 7,
        ...
        self.dim_hi = dim_hi
        self.tim_k = tim_k
        # --- P0 adaptive-attacker hook (backward compatible) ---
        # Let the attack put the deployed purification op (e.g. JPEG-q40) into the
        # EOT loop via env vars, so gradients flow through the exact transform the
        # purifier applies. Unset => original non-adaptive q85, p=p behaviour.
        import os as _os
        _q = _os.environ.get("COGEO_ENVSIM_JPEG_Q")
        if _q:
            jpeg_quality = int(_q)
        _jp = _os.environ.get("COGEO_ENVSIM_JPEG_P")
        self.jpeg_p = float(_jp) if _jp else self.p
