...
\paragraph{Aggregate-mean dominance is significant} CoGEO's $+19.86$ mean rank-lift is $1.6\times$ the next-best baseline, with every pairwise paired Wilcoxon $p$-value well below $0.05$ (to $3.5 \times 10^{-30}$ versus AdvCLIP) on this $560$-pair set --- though the CoGEO$-$PGD-bare margin specifically attenuates to non-significance at the $1{,}430$-pair scale-up ($p{=}0.78$, \S\ref{sec:analysis:summary}). AdvCLIP's universal patch~\cite{zhou2023advclip} moves the ranking least --- a single $\delta$ against a fixed text concept does not adapt to per-image content~\cite{moosavidezfooli2017uap} --- and Co-Attack falls statistically below PGD-bare ($p = 2.4 \times 10^{-6}$), its text-anchor branch not compensating for the loss of CoGEO's mask and momentum~\cite{dong2018mifgsm,xie2019dim,lin2020sim}.
